...
plot.post.prior <- function(w, wsd, rlen, rRes, rlo, rhi, rplotlo, rplothi) {
  ...
  txtout <- array(c(rplot,dpost),dim = c(length(rplot),2))
  write.table(txtout, 'bj_post.txt', append = FALSE, sep = " ", dec = ".",
              row.names = FALSE, col.names = FALSE)
  write.table(rRes, 'bj_post_res.txt', append = FALSE, sep = " ", dec = ".",
              row.names = FALSE, col.names = FALSE)
